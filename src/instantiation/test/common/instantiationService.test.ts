...
import { createDecorator, IInstantiationService, optional, ServicesAccessor } from 'yuzhi/instantiation/common/instantiation';
...
suite('Instantiation Service', () => {
	...
	test('SyncDesc - explode on loop', function () {
		...
		try {
			...
			assert.ok((err as any).name);
			assert.ok((err as any).message);
		}
	});
