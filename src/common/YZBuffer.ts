...
export class YZBuffer {
  ...
  getInt32(index: number): number {
    return (
      this._content[index] |
      (this._content[index + 1] << 8) |
      (this._content[index + 2] << 16) |
      (this._content[index + 3] << 24)
    );
  }
  ...
  getInt64(index: number): number {
    return (
      ...
      (this._content[index + 2] << 16) |
      (this._content[index + 3] << 24) |
      (this._content[index + 4] << 32) |
      (this._content[index + 5] << 40) |
      (this._content[index + 6] << 48) |
      (this._content[index + 7] << 56)
    );
  }
