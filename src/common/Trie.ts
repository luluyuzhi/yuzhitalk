  ...
  protected searchNext(node, stringData) {
    ...
    if (stringData[0] != node.key) {
      ...
      if (children.length == 0 && stringData.length == 1) {
        ...
        // C1：叶子节点，C2：最后一个字符；若只满足其中一个条件，则不匹配
        return false;
      }
    }
  }
...
export class Trie extends TrieBase implements ITrie {
  ...
  //删除单词
  override deleteData(stringData): Trie {
  ...
  printData(): void {
    for (let i in this.root.children) {
      ...
      this.printHelper(this.root.children[i], [
        (<TrieNode>this.root.children[i]).key,
      ]);
    }
  }
}
