...
export function MiddleProtocol2TranslationBuffer(
  MiddleProtocol: IMiddleProtocol
): TranslationBuffer {
...
export function TranslationBuffer2MiddleProtocol(
  translationBuffer: TranslationBuffer
): IMiddleProtocol {
  const version = new StringDecoder("ascii").write(
    translationBuffer.slice(0, 13)
  ) as EVersion;
