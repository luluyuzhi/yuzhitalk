...
export function encodeAuthStatus(message: AuthStatus): Uint8Array {
  let bb = popByteBuffer();
  _encodeAuthStatus(message, bb);
  return toUint8Array(bb);
}

function _encodeAuthStatus(message: AuthStatus, bb: ByteBuffer): void {
...
export function decodeAuthStatus(binary: Uint8Array): AuthStatus {
  return _decodeAuthStatus(wrapByteBuffer(binary));
}

function _decodeAuthStatus(bb: ByteBuffer): AuthStatus {
  let message: AuthStatus = {} as any;
...
function _encodeAuthUser(message: AuthUser, bb: ByteBuffer): void {
  ...
  // required AuthToken secretKey = 2;
  let $secretKey = message.secretKey;
  if ($secretKey !== undefined) {
    writeVarint32(bb, 18);
    let nested = popByteBuffer();
    _encodeAuthToken($secretKey, nested);
  ...
  end_of_message: while (!isAtEnd(bb)) {
    ...
    switch (tag >>> 3) {
      ...
      case 2: {
        ...
        message.secretKey = _decodeAuthToken(bb);
        bb.limit = limit;
        break;
      }
